...
})<CategoryIconProps>(({ theme, accentColor }) => ({
  position: 'absolute',
  top: -16,
  left: 16,
  width: 32,
...
interface CustomerStatisticsModalProps {
  open?: boolean;
  statistics?: DashboardStatsData | null;
  onClose: () => void;
}

export const CustomerStatisticsModal = ({ open = false, statistics, onClose }: CustomerStatisticsModalProps) => {
  ...
  const pieData = {
    ...
    datasets: [
      {
        data: [
          individualData.isError ? 0 : Number(individualData.value) || 0,
          factoryData.isError ? 0 : Number(factoryData.value) || 0,
        ],
          ...
          {activeTab === 1 && (
            <TabPanel>
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <MainCard>
                    <CategoryIcon accentColor="success">
                      <Business />
                    </CategoryIcon>
