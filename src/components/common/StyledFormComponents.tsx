...
import { TextField, Autocomplete, FormControl, type TextFieldProps, type AutocompleteProps } from '@mui/material';

// Shared Styled Components για Form Elements

interface StyledTextFieldExtraProps {
  isMultiline?: boolean;
}

const BaseTextField = ({ inputProps, ...otherProps }: TextFieldProps) => (
  <TextField
    {...otherProps}
    autoComplete="off"
    inputProps={{
...
})<StyledTextFieldExtraProps>(({ theme, size = 'medium', isMultiline }) => ({
...
export const StyledAutocomplete = styled(BaseAutocomplete)(({ theme, size = 'medium' }) => ({
  '& .MuiOutlinedInput-root': {
    borderRadius: '12px',
    backgroundColor: 'rgba(255, 255, 255, 0.8)',
    backdropFilter: 'blur(10px)',
