...
import { Box, Typography, Tooltip, type SxProps, type Theme } from '@mui/material';
...
const dashBetweenSx: SxProps<Theme> = {
  position: 'relative',
  '&::after': {
    content: '"–"',
    position: 'absolute',
...
interface GroupRow {
  ...
  label: string;
  sequence?: boolean;
  get: (group: WindingGroup) => ReactNode;
}

interface CombinedBoxInfoDisplayProps {
 ...
 */
function CombinedBoxInfoDisplay(props: CombinedBoxInfoDisplayProps) {
  ...
  const groupRows: GroupRow[] = [
    { key: 'step', label: 'Βήμα:', sequence: true, get: (group: WindingGroup) => group.step },
    groups.some((group) => group.spiral) && {
      key: 'spiral',
      label: 'Σπείρες:',
      sequence: true,
      get: (group: WindingGroup) => group.spiral,
    },
    groups.some((group) => group.crossSection) && {
      key: 'crossSection',
      label: 'Διατομή:',
      get: (group: WindingGroup) => group.crossSection,
    },
