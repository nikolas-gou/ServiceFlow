...
function CreateConnectionForm({ onSubmitSuccess }) {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'poles') {
      setConnection((prev) => ({
        ...prev,
        poles: value,
        rpm: poles_types_mapping_to_rpm[value] || prev.rpm,
      }));
    } else if (name === 'rpm') {
      setConnection((prev) => ({
        ...prev,
        rpm: value,
        poles: rpm_types_mapping_to_poles[value] || prev.poles,
      }));
    } else {
      setConnection((prev) => ({
        ...prev,
