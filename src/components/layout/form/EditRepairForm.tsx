...
import { uploadSize, type UploadCandidate } from '../../Models/Image';
...
const normalizeRepair = (initialRepair?: RepairJSON | null): RepairFormState => ({
  ...(initialRepair || {}),
  customer: initialRepair?.customer || {},
  motor: initialRepair?.motor || {},
});
...
function EditRepairForm({ repair: initialRepair, onSubmitSuccess, onDirtyChange }: EditRepairFormProps) {
  ...
  // STATES
  const [repair, setRepair] = useState<RepairFormState>(() => normalizeRepair(initialRepair));
  ...
  const [filesToDelete, setFilesToDelete] = useState<Array<number | string>>([]);
  const [isDirty, setIsDirty] = useState(false);
  const initialSnapshotRef = useRef(initialRepair ? JSON.stringify(initialRepair) : '');

  useEffect(() => {
    if (initialRepair) {
      setRepair(normalizeRepair(initialRepair));
      try {
      ...
      if (child === 'hp') {
        ...
        setRepair((prev) => ({
          ...
          [parent]: {
            ...prev[parent],
            hp: processedValue,
            kw: (parseFloat(String(processedValue)) * 0.745699872).toFixed(2), // Μετατροπή hp σε kw
          },
        }));
      } else if (child === 'kw') {
        ...
        setRepair((prev) => ({
          ...
          [parent]: {
            ...prev[parent],
            kw: processedValue,
            hp: (parseFloat(String(processedValue)) / 0.745699872).toFixed(2), // Μετατροπή kw σε hp
          },
        }));
      } else if (child === 'rpm') {
  ...
  const handleSubmit = async (e?: FormEvent) => {
    ...
    if (validateAllTabs()) {
      ...
      try {
        ...
        const repairId = repair.id as number | string;
        await RepairRepository.updateRepair(repairId, {
          repair: dataApi,
        });
        ...
        if (dataApi.images.length > 0 && dataApi.images.some((image) => image.id === null)) {
          const filesToUpload = dataApi.images
            .filter((image) => image.id === null && image.file !== null)
            .map((image) => image.file as File);
          await ImageRepository.uploadImages(filesToUpload, repairId);
        }
        ...
        await RepairRepository.getRepairById(dataApi.id as number | string);

        // Ενημέρωση του state (η πραγματική λίστα διαχειρίζεται από το React Query cache)
        setRepairs();
        ...
        const message = err instanceof Error ? err.message : 'Προέκυψε σφάλμα κατά την ενημέρωση της επισκευής';
  ...
  const getStepIcon = (stepIndex: number, activeStep: number) => {
