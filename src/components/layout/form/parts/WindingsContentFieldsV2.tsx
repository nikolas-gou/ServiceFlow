...
import SimpleWindingBuilder, { type SimpleFieldNames } from './winding/parts/SimpleWindingBuilder';
...
const FIELDS: Record<'main' | 'helper', Record<'standard' | 'half', SimpleFieldNames>> = {
  main: {
...
const CROSS: Record<'main' | 'helper', Record<'standard' | 'half', string>> = {
  main: { standard: 'main_standard', half: 'main_half' },
  helper: { standard: 'helper_standard', half: 'helper_half' },
};

type WindingsContentFieldsV2Props = Pick<RepairFormBagProps, 'repair' | 'setRepair' | 'handleInputChange'>;

function WindingsContentFieldsV2({ repair, setRepair, handleInputChange }: WindingsContentFieldsV2Props) {
  ...
  const twoCoils = (left: ReactNode, right: ReactNode) => (
