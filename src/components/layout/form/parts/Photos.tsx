...
import { Image, type ImageInput } from '../../../Models/Image';
import config from '../../../../config';
import { compressImages } from '../../../../utils/imageCompression';
import type { RepairFormState, RepairFormSetter } from '../../../../types/repairForm';
...
interface PhotosProps {
  ...
  setFilesToDelete: (updater: (prev: Array<number | string>) => Array<number | string>) => void;
  onError?: (message: string) => void;
}

const Photos = ({ repair, setRepair, setFilesToDelete, onError }: PhotosProps) => {
    ...
    if (files && files.length > 0) {
      ...
      try {
        ...
        if (onError) {
          ...
          onError(message || 'Σφάλμα κατά την επεξεργασία των φωτογραφιών. Παρακαλώ δοκιμάστε ξανά.');
        }
      } finally {
        setIsLoading(false);
  ...
  const handleDeletePhoto = (index: number) => {
    ...
    if (!photoToDelete.isNew && photoToDelete.id != null) {
      setFilesToDelete((prev) => [...prev, photoToDelete.id as number | string]);
    }
  };

  const getImageUrl = (image: Image | ImageInput): string => {
    ...
    <PhotoUploadContainer>
      ...
      <PhotoPreviewGrid container spacing={2}>
        ...
        {photos.map((photo, index) => (
          <Grid item xs={12} sm={6} md={4} key={photo.preview ?? photo.id ?? index}>
            <PhotoPreview>
