...
function StepField(props: StepFieldProps) {
  ...
  const handleStepChange = (event: unknown, newValue: string | null) => {
  ...
  useEffect(() => {
    const stepValue = String(props.step_value ?? '');
    if ((stepValue && validateStepPattern(stepValue)) || stepValue === '') {
      setStepError('');
    } else {
      setStepError(
