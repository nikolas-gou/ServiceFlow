...
import { Autocomplete, TextField, Chip, Box, Typography, Grid, type SxProps, type Theme } from '@mui/material';
...
interface CrossSectionFieldProps {
...
export const CrossSectionField = (props: CrossSectionFieldProps) => {
  const { suggested } = useSuggestedFormValues();
  const { motor } = suggested;
  const [crossSections, setCrossSections] = useState<string[]>(motor.crossSection.data);

  /**
   * Regex for validation - Acceptable formats: Χ/10,  Χ.Χ/10, X.XX/10.
   */
  const validateCrossSectionPattern = (value: string): boolean => {
  ...
  const handleAddCrossSection = (event: unknown, newValue: string | null) => {
  ...
  const handleDeleteChip = (sectionToDelete: string) => {
    props.setRepair((prev) => {
      const currentLinks = Array.isArray(prev.motor.motorCrossSectionLinks)
        ? prev.motor.motorCrossSectionLinks
        : [];
    ...
    <Grid container spacing={2} sx={props.sx && props.sx}>
      <Grid item xs={12} sm={12}>
        <Autocomplete
          ...
          renderInput={(params) => {
            const currentInput = (params.inputProps.value as string) || '';
        ...
        {selectedLinks.length > 0 && (
          <Box sx={{ mt: 2 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
              {Object.entries(
                selectedLinks.reduce<Record<string, number>>((acc, link) => {
                  const section = String(link.crossSection);
                  acc[section] = (acc[section] || 0) + 1;
                  return acc;
                }, {}),
