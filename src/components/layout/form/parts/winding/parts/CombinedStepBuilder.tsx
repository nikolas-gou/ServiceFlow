...
interface StepCardProps {
  label: ReactNode;
  isHalf: boolean;
  onToggleHalf: (isHalf: boolean) => void;
  spiralField: ReactNode;
  extra?: ReactNode;
}
 ...
 */
function StepCard({ label, isHalf, onToggleHalf, spiralField, extra }: StepCardProps) {
...
export interface CombinedFieldNames {
...
const DEFAULT_FIELDS: CombinedFieldNames = {
  step: 'step',
  halfStep: 'halfStep',
  spiral: 'spiral',
...
interface CombinedStepBuilderProps {
  repair: RepairFormState;
  setRepair: RepairFormSetter;
  handleInputChange: InputChangeHandler;
  fields?: CombinedFieldNames;
 ...
 *
 * @param half - true όταν εμφανίζεται σε μισό πλάτος (μονοφασικό side-by-side):
...
function CombinedStepBuilder({
  ...
  const fieldName = (key: keyof CombinedFieldNames) => `motor.${F[key]}`;
  const stepValue = String(motor[F.step] || '');
  ...
  const updateSpiralAt = (index: number, value: string) => {
    const next = [...spirals];
  ...
  const setHalfFlag = (token: string, isHalf: boolean) => {
  ...
  const setPairHalfFlag = (isHalf: boolean) => {
    handleInputChange({ target: { name: fieldName('halfStep'), value: isHalf ? stepValue : '' } });
  };

  const spiralField = (value: string, onChange: (e: ChangeEvent<HTMLInputElement>) => void) => (
    <>
            ...
            {isOneToNPair ? (
              ...
              <Grid item {...cardGridProps}>
                <StepCard
                  ...
                  spiralField={spiralField(String(motor[F.spiral] || ''), (e) =>
