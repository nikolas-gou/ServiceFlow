...
import { common_faults, common_faults_translated, type CommonFaultOption } from '../../../Models/CommonFault';
import { StyledTextField } from '../../../common/StyledFormComponents';
import type { RepairFormBagProps } from '../../../../types/repairForm';

type IssuesProps = Pick<
  RepairFormBagProps,
  'repair' | 'setRepair' | 'errors' | 'setErrors' | 'handleInputChange' | 'hasError' | 'getErrorMessage'
>;

export const Issues = (props: IssuesProps) => {
  const [, setSelectedFault] = useState<number[]>([]);

  // Προσθήκη συνηθισμένων θεμάτων στην περιγραφή
  const handleAddCommonIssue = (common_fault: CommonFaultOption) => {
