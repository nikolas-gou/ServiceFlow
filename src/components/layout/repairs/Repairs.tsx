...
import { useRepairs, type RepairsFilters } from '../../../context/RepairsContext';
...
import { Repair, type RepairJSON } from '../../Models/Repair';
...
type LocalRepairsFilters = Omit<RepairsFilters, 'search'>;
...
export default function Repairs() {
  ...
  const [selectedRepair, setSelectedRepair] = useState<RepairJSON | null>(null);
  ...
  const [localFilters, setLocalFilters] = useState<LocalRepairsFilters>(() => {
  ...
  const handleStatusChange = (repair: RepairJSON, newStatus: string) => {
    const dataApi = new Repair({ ...repair, repairStatus: newStatus });
    updateRepairMutation.mutate({ id: repair.id as number | string, data: { repair: dataApi } });
  };

  const handleFiltersChange = (newFilters: LocalRepairsFilters) => {
  ...
  const handleViewRepair = (repair: RepairJSON) => {
    navigate(`/dashboard/services/${repair.id}`);
  };

  const handleEditRepair = (repair: RepairJSON) => {
