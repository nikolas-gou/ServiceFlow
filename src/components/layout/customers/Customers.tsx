...
const EMPTY_FILTERS: CustomersFilters = { type: '', email: '' };

interface CustomersSorting {
  sortBy: string;
  sortOrder: 'ASC' | 'DESC';
}
...
export default function Customers() {
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<CustomerJSON | null>(null);

  const [filters, setFilters] = useState<CustomersFilters>(EMPTY_FILTERS);
  const [search, setSearch] = useState('');

  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(() => Number(localStorage.getItem('customersPerPage')) || 10);

  const [sorting, setSorting] = useState<CustomersSorting>({ sortBy: 'name', sortOrder: 'ASC' });
  ...
  const handleFiltersChange = (newFilters: CustomersFilters) => {
  ...
  const handleItemsPerPageChange = (newPerPage: number) => {
    ...
    setPage(1);
    localStorage.setItem('customersPerPage', String(newPerPage));
  };
  ...
  // Handle modal open
  const handleOpenModal = (customer: CustomerJSON) => {
