...
import { Box, Typography, Grid, Chip, StepLabel } from '@mui/material';
...
import { uploadSize } from '../components/Models/Image';
...
const choiceCardSx = (theme, selected, locked) => ({
  display: 'flex',
  alignItems: 'center',
  gap: 1.5,
 ...
 * Κάθε option: { value, label, icon, locked }
 */
function ChoiceGroup({ title, options, value, onSelect }) {
  return (
...
const PHASES_LABELS = { '1-phase': 'Μονοφασικός', '3-phase': 'Τριφασικός' };
const SPEED_LABELS = { 1: '1 Ταχύτητα', 2: '2 Ταχύτητες' };
const STEP_TYPE_LABELS = { standard: 'Ολόκληρο', half: 'Μισό-Μισό', combined: 'Συνδυασμός' };
 ...
 */
function RecapPill({ label, value, onClick }) {
  return (
    <Box
      component="button"
      type="button"
...
function RepairFormV2Page() {
      ...
      if (child === 'hp') {
        setRepair((prev) => ({
          ...
          [parent]: {
            ...prev[parent],
            hp: processedValue,
            kw: processedValue > 0 ? (parseFloat(processedValue) * 0.745699872).toFixed(2) : null,
          },
        }));
      } else if (child === 'kw') {
        setRepair((prev) => ({
          ...
          [parent]: {
            ...prev[parent],
            kw: processedValue,
            hp: processedValue > 0 ? (parseFloat(processedValue) / 0.745699872).toFixed(2) : null,
          },
        }));
      } else if (child === 'rpm') {
        setRepair((prev) => ({
  ...
  const handleSelectPhases = (value) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateAllSteps()) {
      ...
      try {
        ...
        setErrorMessage(error.message || 'Σφάλμα κατά την αποθήκευση. Παρακαλώ δοκιμάστε ξανά.');
        setErrorAlert(true);
      } finally {
